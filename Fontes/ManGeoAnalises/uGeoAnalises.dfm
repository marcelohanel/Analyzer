object frmGeoAnalises: TfrmGeoAnalises
  Left = 15
  Top = 90
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'GeoAnalises'
  ClientHeight = 440
  ClientWidth = 682
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  DesignSize = (
    682
    440)
  PixelsPerInch = 96
  TextHeight = 13
  object SpeedButton2: TcxButton
    Left = 451
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Criar'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      CAB6EACAB6EACAB6E3C2B1A56B5FFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 4
    OnClick = SpeedButton2Click
  end
  object SpeedButton3: TcxButton
    Left = 483
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Alterar'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      A887DCA887B8857AFF00FFFF00FFFF00FF3E4BDB192DC4FF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 5
    OnClick = SpeedButton3Click
  end
  object SpeedButton4: TcxButton
    Left = 515
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Apagar'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 6
    OnClick = SpeedButton4Click
  end
  object cxButton2: TcxButton
    Left = 547
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Atualizar'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      926DD1926DD1926DD1926DA56B5FFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 7
    OnClick = cxButton2Click
  end
  object cxButton3: TcxButton
    Left = 579
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Definir Grupo'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      4B00004B00FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 9
    OnClick = cxButton3Click
  end
  object SpeedButton7: TcxButton
    Left = 611
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Permiss'#245'es'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      AFCA35A7CCFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 11
    OnClick = SpeedButton7Click
  end
  object grd1: TcxGrid
    Left = 8
    Top = 8
    Width = 345
    Height = 378
    TabOrder = 0
    object grd1DBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      FilterBox.CustomizeDialog = False
      DataController.DataSource = dtsGeoAnalises
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsBehavior.IncSearch = True
      OptionsBehavior.IncSearchItem = grd1DBTableView1DBColumn1
      OptionsCustomize.ColumnFiltering = False
      OptionsCustomize.ColumnGrouping = False
      OptionsData.CancelOnExit = False
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsView.ColumnAutoWidth = True
      OptionsView.GroupByBox = False
      OptionsView.Indicator = True
      object grd1DBTableView1DBColumn1: TcxGridDBColumn
        Caption = 'Nome'
        DataBinding.FieldName = 'ID_GEO'
        SortIndex = 0
        SortOrder = soAscending
        Width = 216
      end
      object grd1DBTableView1Column1: TcxGridDBColumn
        Caption = 'Grupo'
        DataBinding.FieldName = 'GRUPO'
        Width = 113
      end
    end
    object grd1Level1: TcxGridLevel
      GridView = grd1DBTableView1
    end
  end
  object DBEdit1: TcxDBTextEdit
    Left = 360
    Top = 24
    TabStop = False
    DataBinding.DataField = 'ID_GEO'
    DataBinding.DataSource = dtsGeoAnalises
    TabOrder = 1
    Width = 315
  end
  object DBMemo2: TcxDBMemo
    Left = 360
    Top = 113
    TabStop = False
    DataBinding.DataField = 'DESCRICAO'
    DataBinding.DataSource = dtsGeoAnalises
    Properties.ScrollBars = ssBoth
    TabOrder = 3
    Height = 273
    Width = 313
  end
  object Label1: TcxLabel
    Left = 360
    Top = 8
    Caption = 'Nome'
    Transparent = True
  end
  object Label4: TcxLabel
    Left = 360
    Top = 96
    Caption = 'Descri'#231#227'o'
    Transparent = True
  end
  object SpeedButton1: TcxButton
    Left = 643
    Top = 399
    Width = 33
    Height = 33
    Hint = 'Sair do Formul'#225'rio'
    Anchors = [akRight, akBottom]
    OptionsImage.Glyph.Data = {
      
      4B4B4E1E1FFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    ParentShowHint = False
    ShowHint = True
    TabOrder = 13
    OnClick = SpeedButton1Click
  end
  object cxLabel1: TcxLabel
    Left = 360
    Top = 52
    Caption = 'Grupo'
    Transparent = True
  end
  object cxDBTextEdit1: TcxDBTextEdit
    Left = 360
    Top = 68
    TabStop = False
    DataBinding.DataField = 'GRUPO'
    DataBinding.DataSource = dtsGeoAnalises
    TabOrder = 2
    Width = 315
  end
  object dtsGeoAnalises: TDataSource
    AutoEdit = False
    DataSet = tblGeoAnalises
    Left = 624
    Top = 8
  end
  object CEVersionInfo1: TCEVersionInfo
    Left = 64
    Top = 72
  end
  object cxLocalizer1: TcxLocalizer
    Left = 136
    Top = 72
  end
  object dxSkinController1: TdxSkinController
    Kind = lfStandard
    NativeStyle = False
    SkinName = 'Black'
    UseSkins = False
    Left = 208
    Top = 72
  end
  object bdArmazena: TADOConnection
    ConnectionString = 
      'Provider=Microsoft.ACE.OLEDB.12.0;Data Source=c:\analyzer\analyz' +
      'er.accdb;Persist Security Info=False;'
    CursorLocation = clUseServer
    KeepConnection = False
    LoginPrompt = False
    Provider = 'Microsoft.ACE.OLEDB.12.0'
    Left = 136
    Top = 132
  end
  object bdDados: TADOConnection
    ConnectionString = 
      'Provider=SQLNCLI11.1;Integrated Security="";Persist Security Inf' +
      'o=False;User ID=sa;Initial Catalog=AnalyzerSQL;Data Source=local' +
      'host;Initial File Name="";Server SPN="";'
    CursorLocation = clUseServer
    KeepConnection = False
    LoginPrompt = False
    Mode = cmShareDenyNone
    Provider = 'SQLNCLI11.1'
    Left = 64
    Top = 132
  end
  object qryAtualiza: TADOQuery
    Connection = bdDados
    CursorLocation = clUseServer
    Parameters = <>
    Left = 480
    Top = 128
  end
  object tblGeoAnalises: TADOTable
    Connection = bdDados
    CursorLocation = clUseServer
    BeforePost = tblGeoAnalisesBeforePost
    BeforeDelete = tblGeoAnalisesBeforeDelete
    OnFilterRecord = tblGeoAnalisesFilterRecord
    TableName = 'GEO_ANALISES'
    Left = 552
    Top = 8
    object tblGeoAnalisesID_GEO: TWideStringField
      FieldName = 'ID_GEO'
      Size = 50
    end
    object tblGeoAnalisesDESCRICAO: TWideMemoField
      FieldName = 'DESCRICAO'
      BlobType = ftWideMemo
    end
    object tblGeoAnalisesARQUIVO: TBlobField
      FieldName = 'ARQUIVO'
    end
    object tblGeoAnalisesPODE_VISUALIZAR: TWideMemoField
      FieldName = 'PODE_VISUALIZAR'
      BlobType = ftWideMemo
    end
    object tblGeoAnalisesPODE_ALTERAR: TWideMemoField
      FieldName = 'PODE_ALTERAR'
      BlobType = ftWideMemo
    end
    object tblGeoAnalisesPODE_APAGAR: TWideMemoField
      FieldName = 'PODE_APAGAR'
      BlobType = ftWideMemo
    end
    object tblGeoAnalisesDT_CRIACAO: TDateTimeField
      FieldName = 'DT_CRIACAO'
    end
    object tblGeoAnalisesGRUPO: TWideStringField
      FieldName = 'GRUPO'
      Size = 50
    end
  end
end
