object frmSelecaoGraficoScored: TfrmSelecaoGraficoScored
  Left = -7
  Top = 28
  Caption = 'Selecionar dados para o Gr'#225'fico'
  ClientHeight = 483
  ClientWidth = 711
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 39
    Width = 711
    Height = 444
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    object cxGrid1: TcxGrid
      Left = 0
      Top = 0
      Width = 711
      Height = 444
      Align = alClient
      TabOrder = 0
      object cxGrid1DBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.Insert.Visible = False
        Navigator.Buttons.Delete.Visible = False
        Navigator.Buttons.Edit.Visible = False
        Navigator.Buttons.Post.Visible = False
        Navigator.Buttons.Refresh.Visible = False
        Navigator.Buttons.SaveBookmark.Visible = False
        Navigator.Buttons.GotoBookmark.Visible = False
        Navigator.Visible = True
        DataController.DataModeController.SyncMode = False
        DataController.DataSource = DataSource1
        DataController.Options = [dcoAnsiSort, dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding, dcoSortByDisplayText, dcoFocusTopRowAfterSorting]
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsBehavior.IncSearch = True
        OptionsCustomize.ColumnGrouping = False
        OptionsCustomize.ColumnMoving = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.Footer = True
        OptionsView.GroupByBox = False
        OptionsView.GroupFooters = gfAlwaysVisible
        object cxGrid1DBTableView1_Atualizacao_: TcxGridDBColumn
          Caption = 'Atualiza'#231#227'o'
          DataBinding.FieldName = '_Atualizacao_'
          Width = 80
        end
        object cxGrid1DBTableView1_Data_: TcxGridDBColumn
          Caption = 'Data'
          DataBinding.FieldName = '_Data_'
          RepositoryItem = frmMostraScored.cxEditRepository1DateItem1
          Width = 80
        end
        object cxGrid1DBTableView1_Valor_: TcxGridDBColumn
          Caption = 'Valor'
          DataBinding.FieldName = '_Valor_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem1
          Width = 80
        end
        object cxGrid1DBTableView1_EvolucaoAnterior_: TcxGridDBColumn
          Caption = '% Evol.Anterior'
          DataBinding.FieldName = '_EvolucaoAnterior_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_EvolucaoOriginal_: TcxGridDBColumn
          Caption = '% Evol.Original'
          DataBinding.FieldName = '_EvolucaoOriginal_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_ValorMinimo_: TcxGridDBColumn
          Caption = 'Valor M'#237'nimo'
          DataBinding.FieldName = '_ValorMinimo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem1
          Width = 80
        end
        object cxGrid1DBTableView1_ValorMaximo_: TcxGridDBColumn
          Caption = 'Valor M'#225'ximo'
          DataBinding.FieldName = '_ValorMaximo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem1
          Width = 80
        end
        object cxGrid1DBTableView1_PercMinimo_: TcxGridDBColumn
          Caption = '% M'#237'nimo'
          DataBinding.FieldName = '_PercMinimo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_PercMaximo_: TcxGridDBColumn
          Caption = '% M'#225'ximo'
          DataBinding.FieldName = '_PercMaximo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_ValorFaltaMinimo_: TcxGridDBColumn
          Caption = 'Vlr.Falta M'#237'nimo'
          DataBinding.FieldName = '_ValorFaltaMinimo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem1
          Width = 80
        end
        object cxGrid1DBTableView1_ValorFaltaMaximo_: TcxGridDBColumn
          Caption = 'Vlr.Falta M'#225'ximo'
          DataBinding.FieldName = '_ValorFaltaMaximo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem1
          Width = 80
        end
        object cxGrid1DBTableView1_PercFaltaMinimo_: TcxGridDBColumn
          Caption = '% Falta M'#237'nimo'
          DataBinding.FieldName = '_PercFaltaMinimo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_PercFaltaMaximo_: TcxGridDBColumn
          Caption = '% Falta M'#225'ximo'
          DataBinding.FieldName = '_PercFaltaMaximo_'
          RepositoryItem = frmMostraScored.cxEditRepository1CurrencyItem2
          Width = 80
        end
        object cxGrid1DBTableView1_Situacao_: TcxGridDBColumn
          Caption = 'Situa'#231#227'o'
          DataBinding.FieldName = '_Situacao_'
          RepositoryItem = frmMostraScored.cxEditRepository1ImageComboBoxItem4
          Width = 80
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
  end
  object Panel5: TPanel
    Left = 0
    Top = 0
    Width = 711
    Height = 39
    Align = alTop
    TabOrder = 0
    DesignSize = (
      711
      39)
    object Label1: TcxLabel
      Left = 4
      Top = 11
      Caption = 'Estilos:'
      Transparent = True
    end
    object ComboBox1: TcxComboBox
      Left = 41
      Top = 10
      Properties.DropDownListStyle = lsEditFixedList
      Properties.OnChange = ComboBox1PropertiesChange
      TabOrder = 8
      Width = 114
    end
    object cxRadioGroup1: TcxRadioGroup
      Left = 158
      Top = -3
      Properties.Columns = 5
      Properties.Items = <
        item
          Caption = 'Dia'
        end
        item
          Caption = 'M'#234's'
        end
        item
          Caption = 'Trimestre'
        end
        item
          Caption = 'Semestre'
        end
        item
          Caption = 'Ano'
        end>
      ItemIndex = 0
      TabOrder = 0
      Transparent = True
      Height = 38
      Width = 333
    end
    object SpeedButton9: TcxButton
      Left = 516
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Salvar Cen'#225'rio Atual'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        C0C0C4C0C0C4C0C0C4C0C0C4C0C0C6C7C7BC99988C3435FF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = SpeedButton9Click
    end
    object cxButton2: TcxButton
      Left = 548
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Enviar as informa'#231#245'es para o Excel'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        906BA5696ABB7F6AFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = cxButton2Click
    end
    object SpeedButton6: TcxButton
      Left = 580
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Enviar as informa'#231#245'es por E-Mail'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        0A18180A5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = SpeedButton6Click
    end
    object SpeedButton4: TcxButton
      Left = 612
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Imprimir as informa'#231#245'es'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        261E1A123B3B3B3B3B3B3B3B3B1111111112123B3B3B3B3B3B3B}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnClick = SpeedButton4Click
    end
    object SpeedButton1: TcxButton
      Left = 644
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Cancelar as modifica'#231#245'es'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        60CFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = SpeedButton1Click
    end
    object SpeedButton3: TcxButton
      Left = 676
      Top = 3
      Width = 33
      Height = 33
      Hint = 'Confirmar as modifica'#231#245'es'
      Anchors = [akTop, akRight]
      OptionsImage.Glyph.Data = {
        
        3D07FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = SpeedButton3Click
    end
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    PreviewOptions.ShowExplorer = True
    PreviewOptions.WindowState = wsMaximized
    Version = 0
    Left = 256
    Top = 88
    object dxComponentPrinterLink1: TdxGridReportLink
      Component = cxGrid1
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 12700
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportDocument.CreationDate = 38835.692293287030000000
      ReportDocument.Creator = 'mhanel'
      ReportTitle.Font.Charset = DEFAULT_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -19
      ReportTitle.Font.Name = 'Times New Roman'
      ReportTitle.Font.Style = [fsBold]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Times New Roman'
      Font.Style = []
      OptionsCards.Shadow.Color = clBlack
      BuiltInReportLink = True
    end
  end
  object cxGridPopupMenu1: TcxGridPopupMenu
    Grid = cxGrid1
    PopupMenus = <>
    Left = 368
    Top = 80
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = '*.xls'
    Filter = 'Excel|*.xls'
    Left = 224
    Top = 88
  end
  object ADOQuery1: TADOQuery
    Connection = frmMostraScored.bdArmazena
    CursorLocation = clUseServer
    Parameters = <>
    SQL.Strings = (
      'SELECT '
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_Atualizacao_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_Data_], '
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_Valor_], '
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_EvolucaoAnterior_], '
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_EvolucaoOriginal_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_ValorMinimo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_ValorMaximo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_PercMinimo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_PercMaximo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_ValorFaltaMinimo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_ValorFaltaMaximo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_PercFaltaMinimo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_PercFaltaMaximo_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_Situacao_],'
      '[_ScoredCard_AN'#225'LISE FINANCEIRA].[_Situacao_] AS [_Situacao1_]'
      'FROM [_ScoredCard_AN'#225'LISE FINANCEIRA];')
    Left = 408
    Top = 80
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 448
    Top = 78
  end
end
